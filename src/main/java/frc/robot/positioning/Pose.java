...
/** Keeps track of a position and orientation with most of the gross math out of the way */
public class Pose extends Pose3d {
  ...
  /** Constructs a Pose at x,y,z with an orientation described by roll,pitch,yaw in degrees */
  public Pose(final double x, final double y, final double z, final Rotation3d rotation) {
    super(x, y, z, rotation);
  }

  /**
   * Returns this pose as a wpilib Pose3d This doesn't lose any information because the Pose class
   ...
   * this method will give you a new pose with reference to the world instead of the robot.
   *
   * <p>This method can be chained if you want to find the pose of an arm in the world. If the arm
   * pose is based at the bottom of the robot, and the robot can move around in the world, you can
   * chain to find world coordinates. First, you would find convert from arm coordinates to robot
   * coordinates. Then, you would convert robot coordinates to world coordinates.
   *
  ...
  /**
   * Pitches the current pose up/down 3D rotations can get confusing quick, this will pitch the pose
   * as if it were a plane flying in its starting orientation.
   *
  ...
  /**
   * Rolls the current pose counter clockwise 3D rotations can get confusing quick, this will roll
   * the pose as if it were a plane flying in its starting orientation.
   *
  ...
  /**
   * Rotates the pose around the x, y, and z axes 3D rotations can get confusing quick, this will
   * change the pose in the same way no matter how it's oriented beforehand.
   */
  ...
  /**
   * Yaws the current pose left/right 3D rotations can get confusing quick, this will yaw the pose
   * as if it were a plane flying in its starting orientation.
   *
