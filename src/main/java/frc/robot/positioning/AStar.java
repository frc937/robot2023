...
public class AStar {
    ...
    for (int i = 0; i < matrix.length; i++) {
      for (int j = 0; j < matrix.length; j++) {
        ...
        if (matrix[i][j]) {
          ...
          // TODO: IF THE THING IS RUNNING SLOW, REMOVE THE Math.sqrt() TO MAYBE IMPROVE PROCESSING
          // PROWER
  ...
  private ArrayList<Node> generatePath(
    ...
    while (true) {
      ...
      // Gets and removes the objects that's stored on the top of the openList and saves it inside
      // node
      ...
      // Checks if whether the node returned is having the same node object values of the ending
      // point
    ...
    // Checks if whether the endNode variable currently has a parent Node. if it doesn't then stops
    // moving forward.
